...
function first() {
  ...
  return second; //함수 본문을 내보내기
}
//second 함수를 강제로 빼서 쓰는것이 closure

//curring function
const value = first(); //15

//⭐result에 함수 자체를 할당했기 때문에 가비지컬렉터가 수집대상 제외 !!
//⭐참조가 가능해서 못버림
function counter() {
  let count = 0;
...
let result = counter(); //⭐counter()가 값은 계속 result에 차곡차곡
